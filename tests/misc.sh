...
expect()
{
	...
	if [ $? -eq 0 ]; then
		...
		if [ -z "${todomsg}" ]; then
			...
			echo "not ok ${ntest} - tried 'expect ${e}  $*', expected ${e}, got ${r} # TODO ${todomsg}"
		fi
	fi
	todomsg=""
	ntest=$((ntest+1))
...
jexpect()
{
	...
	if [ $? -eq 0 ]; then
		...
		if [ -z "${todomsg}" ]; then
			...
			echo "not ok ${ntest} - tried 'jexpect ${e} ${d} ${e} $*', expected ${e}, got ${r} # TODO ${todomsg}"
		fi
	fi
	todomsg=""
	ntest=$((ntest+1))
...
test_check()
{
	if [ $* ]; then
		...
		if [ -z "${todomsg}" ]; then
			...
			echo "not ok ${ntest} - tried '$*' # TODO ${todomsg}"
		fi
	fi
	todomsg=""
	ntest=$((ntest+1))
